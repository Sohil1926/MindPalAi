...
import { deleteJournal, setJournalCover } from '../utils/journalUtils';
import {
  getLastJournalCoverDate,
  setLastJournalCoverDate,
...
const JournalCover = ({ navigation, setShowOnboarding, route }) => {
  ...
  const precheckBeforeGenerate = async () => {
    ...
    const lastJCoverGeneratedDate = await getLastJournalCoverDate();
  ...
  const generateImage = async () => {
    ...
    axios
      ...
      .then(async (response) => {
        ...
        await setJournalCover(route.params.key, response.data.response);
        await setLastJournalCoverDate(new Date());
        // Old implementation
